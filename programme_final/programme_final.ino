...
bool police = false; // En cas d'intervention des forces de l'ordre
bool police_etape = false;
...
void sectionLED() {
  ...
  int nouvelle_num_leds = server.arg("m").toInt();
  int nouvelle_section_actuelle = server.arg("s").toInt();

  num_leds = nouvelle_num_leds;
  ...
  // Mode police (2 sections)
    ...
    for(int i = 0; i < LEDS_TOTAL; i++){
      if(i<30){
        ...
        if(police_etape) leds[i] = CRGB(0, 0, 255);
        else leds[i] = CRGB(255, 0, 0);
      }
    }
    police_etape = !police_etape;
    delay(230); // a noter que y'a toujours le délai après
  }
